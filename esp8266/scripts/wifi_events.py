...
class WifiEventManager:
    ...
    def __init__(self, ap_ssid, ap_password):
        esp.wifi_events.init(stamode_connected=lambda ssid, channel: self.stamode_connected(ssid, channel),
                             stamode_disconnected=lambda ssid, reason: self.stamode_disconnected(ssid, reason),
                             stamode_got_ip=lambda ip, mask, gw: self.stamode_got_ip(ip, mask, gw))
